...
const Contact: React.FC = () => {
  ...
  React.useEffect(() => {
    console.log('Component mounted, resetting isSubmitting to false');
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (response.ok) {
        ...
        setTimeout(() => {
          console.log('Resetting isSubmitted to false after 5 seconds');
          setIsSubmitted(false);
        }, 5000);
      } else {
        const errorText = await response.text();
        console.error('Failed to submit form, response:', response.status, errorText);
      }
    } catch (error) {
            ...
            <div className="relative">
              <form onSubmit={handleSubmit} className="space-y-6">
                ...
                <button
                  ...
                  className="w-full bg-gradient-to-r from-peach to-purple hover:from-purple hover:to-peach text-white px-8 py-4 rounded-lg font-semibold transition-all duration-300 transform hover:scale-105 flex items-center justify-center"
                >
                  {isSubmitting ? (
                    <>
